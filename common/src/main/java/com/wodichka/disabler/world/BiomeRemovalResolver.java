...
public final class BiomeRemovalResolver {
    ...
    public static Holder<Biome> resolveReplacement(Holder<Biome> selectedBiome, Climate.TargetPoint targetPoint, Climate.ParameterList<Holder<Biome>> allowedBiomes) {
        if (!DisablerConfig.hasBlockedBiomes() || !isBlocked(selectedBiome) || allowedBiomes.values().isEmpty()) {
            return selectedBiome;
        }

        Holder<Biome> replacement = allowedBiomes.findValue(targetPoint);
        return isBlocked(replacement) ? selectedBiome : replacement;
    }
    ...
    public static Climate.ParameterList<Holder<Biome>> collectAllowedBiomeParameters(Climate.ParameterList<Holder<Biome>> parameterList) {
        List<Pair<Climate.ParameterPoint, Holder<Biome>>> allowedBiomes = new ArrayList<>();
        for (Pair<Climate.ParameterPoint, Holder<Biome>> entry : parameterList.values()) {
            Holder<Biome> biome = entry.getSecond();
            ResourceLocation biomeId = biome.unwrapKey().map(ResourceKey::location).orElse(null);
            if (!isBlocked(biome) && biomeId != null && !getBiomeExceptionIds().contains(biomeId)) {
                allowedBiomes.add(entry);
            }
        }
        return new Climate.ParameterList<>(List.copyOf(allowedBiomes));
    }
